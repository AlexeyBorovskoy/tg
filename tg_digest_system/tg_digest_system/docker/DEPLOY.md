...
4. **Веб-интерфейс**: http://localhost:8000 (или порт из `WEB_PORT` в `.env`).  
   **Закрыть сервис идентификацией:** в `secrets.env` задайте **`AUTH_OWN_ENABLED=1`**, укажите **BASE_URL** (например `http://93.77.185.71:8000`), **YANDEX_OAUTH_CLIENT_ID**, **YANDEX_OAUTH_CLIENT_SECRET**, **JWT_SECRET** и перезапустите контейнеры. Тогда **весь сервис** (главная, каналы, промпты, API) будет доступен только после входа через Яндекс; без авторизации откроется только страница входа и `/health`.  
   Чтобы отключить проверку, задайте `AUTH_OWN_ENABLED=0`.
...
| **migrate**  | Однократный запуск миграций 001–008 после postgres |
...
При деплое или обновлении кода **все данные сохраняются**:

1. **Остановка перед деплоем** (опционально, для бэкапа или ручной проверки):
   ```bash
   ./deploy.sh --stop
   ```
   Контейнеры останавливаются, volumes (БД, сессия Telethon, логи, медиа) не трогаются.

2. **Обычный деплой** (`./deploy.sh` или `./deploy.sh --build`):
   - Выполняется `docker compose down` **без** `-v` — контейнеры удаляются, **volumes остаются**.
   - Затем `up -d` — миграции (001–008) выполняются поверх существующей БД (только добавление таблиц/колонок, данные не стираются).
   - Web и worker поднимаются с теми же postgres_data, worker_data, worker_logs, worker_media.

**Не используйте** `docker compose down -v` — это удалит все данные в БД и воркера.

---

## Оповещения по дайджестам и работоспособности

Логика оповещений реализована в воркере (`digest_worker.py`):

- **Уведомления по шагам** — по каждому каналу и шагу (загрузка, OCR, дайджест и т.д.) в Telegram можно отправлять сообщения «Новых сообщений нет» или об ошибках.
- **Глобальные ошибки** — при падении воркера отправляется сообщение «Воркер не работает. Что произошло: …».

Чтобы оповещения работали после деплоя:

1. В **secrets.env** или **.env** задайте:
   - **TG_BOT_TOKEN** — токен бота, который будет отправлять уведомления.
   - **TG_STEP_NOTIFY_CHAT_ID** — ID чата (или пользователя), куда слать уведомления (число). Если не задано, используется ID пользователя из сессии Telethon (если доступен).

2. После перезапуска проверьте логи воркера:
   ```bash
   docker compose logs -f worker
   ```
   Убедитесь, что воркер подключается к БД и Telegram и что уведомления приходят в заданный чат.

Работоспособность веб-интерфейса проверяется по эндпоинту **GET /health** (возвращает `{"status":"ok"}`).
...
# Остановка (контейнеры удаляются, volumes сохраняются)
docker compose down

# Остановка с удалением volumes (БД и данные воркера будут удалены — не использовать при обычном деплое!)
...
Отправка дайджестов настраивается **отдельно от списка каналов** — в файле **`config/digest_delivery.json`**. Для каждого подконтрольного чата можно задать:

- **Важный чат** — полный дайджест (текст + файл).
- **Ознакомительный чат** — краткое сообщение (например, до 500 символов), без файла.

Файл по умолчанию ищется рядом с `channels.json` (т.е. в каталоге `config/`). Переменная **`DIGEST_DELIVERY_FILE`** задаёт полный путь к файлу при необходимости.

Подробнее: [docs/НАСТРОЙКИ_ДОСТАВКИ_ДАЙДЖЕСТОВ.md](../docs/НАСТРОЙКИ_ДОСТАВКИ_ДАЙДЖЕСТОВ.md). Пример и схема: `config/digest_delivery.json`, `config/digest_delivery.schema.json`.
...
Миграции лежат в `../db/migrations/` (001–008). Миграция 008 добавляет настройки доставки дайджестов по каналу (важный/ознакомительный) в `web_channels`. Схема БД при первом запуске postgres берётся из `../db/schema.sql`.

---

## Деплой в Yandex Cloud

Полная инструкция и скрипт установки на ВМ в Yandex Cloud — в каталоге **`yandex/`**:

- **yandex/README.md** — создание ВМ, установка Docker, клонирование репо, настройка `BASE_URL` и секретов, запуск `deploy.sh`, открытие порта 8000.
- **yandex/setup-on-vm.sh** — скрипт для запуска на ВМ: проверка Docker, создание `.env`/`secrets.env`, подстановка `BASE_URL` по публичному IP, запуск деплоя.

После деплоя на Yandex ссылка для входа: **http://<ПУБЛИЧНЫЙ_IP_ВМ>:8000/login**
